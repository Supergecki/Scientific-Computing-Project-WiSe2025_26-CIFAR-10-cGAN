...
class Discriminator(nn.Module):
    ...
    def __init__(self, channel_config, num_classes=10, image_size=32, embed_dim=50):
        ...
        self.flatten_layers = nn.Sequential(
            nn.Flatten(),
            nn.Linear(channel_config[-1] * current_image_size * current_image_size + embed_dim, 1)
        )
    def forward(self, x, labels):
        ...
        :param labels:
            torch.Tensor, class labels corresponding to the input images
        :return:
            class logit (x was a real image or not)
        """
        y = self.conv_layers(x)
        y = torch.flatten(y, start_dim=1)  # flatten all dimensions except batch dimension
        label_embed = self.label_embedding(labels)
        y = torch.cat([y, label_embed], dim=1)
